...
function onResults(results) {
  ...
  for(let i = 0; i < body_parts.length; i++) {
    for(let j = 0; j < 3; j++) {
      if((body_parts[i].x >= ((left_tiles[j].x-5)/canvasElement.width) && body_parts[i].x <= (left_tiles[j].x+15)/canvasElement.width) &&
    ...
    for(let j = 0; j < 3; j++) {
      if((body_parts[i].x >= ((right_tiles[j].x-5)/canvasElement.width) && body_parts[i].x <= (right_tiles[j].x+15)/canvasElement.width) &&
